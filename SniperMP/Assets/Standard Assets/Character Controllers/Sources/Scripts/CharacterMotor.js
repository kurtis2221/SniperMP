...
var height_normal : float = 2.25;
var height_crouch : float = 1.25;
private var height_diff : float;
var hit : RaycastHit;
...
class CharacterMotorMovement {
	...
	var maxRunningSpeed : float = 15.0;
...
private function UpdateFunction () {
	...
	if(Input.GetButton("Crouch"))
	{
		...
		{
			maincam.transform.position = Vector3(
	...
	else
	{
		...
		{
			Physics.SphereCast(transform.position + Vector3(0,0.3,0),0.3,transform.up,hit,1.25);
			if(!hit.collider)
			{
				maincam.transform.position = Vector3(
				maincam.transform.position.x,
...
private function ApplyGravityAndJumping (velocity : Vector3) {
	...
	if (grounded) {
		...
		if (stamina > 100 && jumping.enabled && canControl && (Time.time - jumping.lastButtonDownTime < 0.2)) {
...
function MaxSpeedInDirection (desiredMovementDirection : Vector3) : float {
	...
	if(Input.GetButton("Run/Sprint") && stamina > 10 && !Input.GetButton("Crouch"))
	{
		if (desiredMovementDirection == Vector3.zero)
			return 0;
		else {
			zAxisEllipseMultiplier = (desiredMovementDirection.z > 0 ? movement.maxRunningSpeed : movement.maxRunningSpeed) / movement.maxRunningSpeed;
			temp = new Vector3(desiredMovementDirection.x, 0, desiredMovementDirection.z / zAxisEllipseMultiplier).normalized;
			length = new Vector3(temp.x, 0, temp.z * zAxisEllipseMultiplier).magnitude * movement.maxRunningSpeed;
			return length;
		}
	}
	else
